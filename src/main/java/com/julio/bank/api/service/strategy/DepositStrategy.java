...
public class DepositStrategy implements EventStrategy {
    ...
    public DepositStrategy(BalanceService balanceService)
    {
    ...
    {
        Balance destination = balanceService.deposit(request.destination(), request.amount());
        return EventResult.of(null, Account.from(destination));
    }
}
